...
- **独立微服务**：`cmx-rule-server` bin 监听 `:8094`，自持 PostgreSQL 连接，`[auth]` 段（`auth.mode`/`auth.tenancy`，ConfigManager 直读）控认证与租户。
...
- **认证**：`auth.mode=off` 单租户 `default` 放行零回归；开启则验 JWT claim（HS256/RS256）+ scoped 租户上下文（`task_local`）。
